...
#define VARS4UPDATE \
  __m128i k[2], state[S_NUM], M[M_NUM], tmp1, tmp6;
...
#define UPDATE_STATE(X) \
  tmp6 = state[6]; \
  tmp1 = state[1]; \
  state[6] = enc(state[5], state[4]); \
  state[5] = enc(state[4], state[3]); \
  state[4] = enc(state[3],     X[1]); \
  state[3] = enc(state[2],    tmp6 ); \
  state[2] = enc(state[1], state[0]); \
  state[1] = enc(state[0],     X[0]); \
  state[0] = xor(   tmp6 ,    tmp1 ); \
...
void rocca_encrypt(rocca_context * ctx, uint8_t * out, const uint8_t * in, size_t size) {
	...
	COPY_TO_LOCAL(ctx);
	size_t i = 0;
#if defined(ENCRYPT_UNROLL)
	for(size_t size1 = FLOORTO(size, ENCRYPT_UNROLL_PROCESSING_SIZE); i < size1; i += ENCRYPT_UNROLL_PROCESSING_SIZE) {
		ENCRYPT_UNROLL(out + i, in + i);
	}
#endif
	for(size_t size2 = FLOORTO(size, ROCCA_MSG_BLOCK_SIZE); i < size2; i += ROCCA_MSG_BLOCK_SIZE) {
		ENCRYPT(out + i, in + i);
...
void rocca_stream( rocca_context * ctx, uint8_t * out ) {
	VARS4ENCRYPT
	COPY_TO_LOCAL(ctx);
...
void rocca_update_state( rocca_context * ctx, const uint8_t * in, size_t size ){
	VARS4UPDATE
	int msize = size;
	uint8_t tmpblk[ROCCA_MSG_BLOCK_SIZE] = {0};
	...
	if( size > ROCCA_MSG_BLOCK_SIZE ){
		msize = ROCCA_MSG_BLOCK_SIZE;
	}
	memcpy(tmpblk, in, msize);
	MSG_LOAD(tmpblk, M) \
	UPDATE_STATE(M)
...
void rocca_decrypt(rocca_context * ctx, uint8_t * out, const uint8_t * in, size_t size) {
	...
	COPY_TO_LOCAL(ctx);
	size_t i = 0;
#if defined(DECRYPT_UNROLL)
	for(size_t size1 = FLOORTO(size, DECRYPT_UNROLL_PROCESSING_SIZE); i < size1; i += DECRYPT_UNROLL_PROCESSING_SIZE) {
		DECRYPT_UNROLL(out + i, in + i);
	}
#endif
	for(size_t size2 = FLOORTO(size, ROCCA_MSG_BLOCK_SIZE); i < size2; i += ROCCA_MSG_BLOCK_SIZE) {
		DECRYPT(out + i, in + i);
...
void rocca_creanup(rocca_context * ctx) {
	int i, j;
	volatile uint8_t* pkey;
	volatile uint8_t* pstate;

	for( i=0 ; i<(ROCCA_KEY_SIZE/16) ; i++ ){
		pkey = ctx->key[i];
		for( j=0 ; j<16 ; j++ ){
			pkey[j] = 0xff;
		}
	}
	for( i=0 ; i<ROCCA_STATE_NUM ; i++ ){
		pstate = ctx->state[i];
		for( j=0 ; j<16 ; j++ ){
			pstate[j] = 0xff;
		}
